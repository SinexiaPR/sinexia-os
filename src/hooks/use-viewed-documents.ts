...
const LOCAL_VIEWED_PREFIX = "sinexia:document-viewed:";
const VIEWED_EVENT = "sinexia:document-viewed";
...
function useLocalViewedDocumentIds(profileId: string): string[] {
...
export function addLocalViewedDocument(profileId: string, documentId: string) {
  ...
  window.sessionStorage.setItem(
    ...
    JSON.stringify([...current, documentId]),
  );
  window.dispatchEvent(new Event(VIEWED_EVENT));
}

export function useIsDocumentViewed(
  documentId: string,
  viewedDocumentIds: string[],
  profileId: string,
): boolean {
  const localViewed = useLocalViewedDocumentIds(profileId);

  const viewed = useMemo(() => {
    const merged = new Set(viewedDocumentIds);
    for (const id of localViewed) {
      merged.add(id);
    }
    return merged;
  }, [viewedDocumentIds, localViewed]);

  return viewed.has(documentId);
}

export function useUnviewedDocumentsCount(
  profileId: string,
  documentIds: string[],
  viewedDocumentIds: string[],
): number {
  ...
  return useMemo(
    () => documentIds.filter((id) => !viewed.has(id)).length,
    [documentIds, viewed],
  );
}
